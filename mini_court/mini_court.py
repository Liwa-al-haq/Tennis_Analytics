...
class MiniCourt:
    ...
    def add_labels(self, frame, labels):
        for label, (x, y) in labels.items():
            cv2.putText(frame, label, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2, cv2.LINE_AA)
        return frame

    def draw_statistical_elements(self, frame, stats):
        for stat, (x, y, value) in stats.items():
            cv2.putText(frame, f"{stat}: {value}", (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2, cv2.LINE_AA)
        return frame
...
# Example usage:
# Assuming you have the constants and utility functions implemented
if __name__ == "__main__":
    frame = np.zeros((600, 800, 3), dtype=np.uint8)  # Dummy frame for illustration
    visualizer = MiniCourt(frame)
    
    # Dummy data
    player_boxes = [{1: (100, 200, 50, 100), 2: (300, 400, 50, 100)}] * 5
    ball_boxes = [{1: (150, 250, 20, 20)}] * 5
    original_court_key_points = visualizer.get_court_drawing_keypoints()
    
    # Convert bounding boxes to mini court coordinates
    player_positions, ball_positions = visualizer.convert_bounding_boxes_to_mini_court_coordinates(player_boxes, ball_boxes, original_court_key_points)
    
    # Draw mini court with positions
    frames = [frame.copy() for _ in range(5)]
    output_frames = visualizer.draw_mini_court(frames)
    output_frames = visualizer.draw_points_on_mini_court(output_frames, player_positions)
    output_frames = visualizer.draw_points_on_mini_court(output_frames, ball_positions, color=(255, 0, 0))
    
    for i, output_frame in enumerate(output_frames):
        cv2.imshow(f"Mini Court Frame {i}", output_frame)
        cv2.waitKey(0)
